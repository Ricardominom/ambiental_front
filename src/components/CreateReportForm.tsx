...
import { X, Upload, MapPin, Phone, FileText, AlertTriangle, Users } from 'lucide-react';
...
const CreateReportForm: React.FC<CreateReportFormProps> = ({ onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // TEMPORAL: Creación de reporte solo local
    try {
      // Enviar solo los datos del formulario, sin id ni fechaCreacion
      // Estos serán establecidos por el componente padre
      onSubmit(formData);
      onClose();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-start justify-center z-50 p-4 overflow-y-auto modal-scroll">
      <div className="bg-white rounded-b-3xl shadow-2xl w-full max-w-4xl my-8 border border-emerald-100 overflow-hidden" style={{ borderTopLeftRadius: 25, borderTopRightRadius: 25 }}>
        {/* Header */}
        <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white p-8 flex items-center justify-between rounded-t-3xl">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-emerald-100">Completa la información del reporte ambiental</p>
            </div>
          </div>
          <button
            onClick={onClose}
